...
export const BUILDS: Build[] = [
  {
    ...
    prerequisites: ["Any language (Python/TS)", "No API key — the 'decide' step can be a rule"],
    instructions: [
      "Write `calculator(expr)`.",
      "Write `decide(req)` returning either `{tool:'calculator', args}` or `{answer}`.",
      "Run it on '(3+4)*2' and on 'hello'; print the decision and the result each time.",
    ],
  ...
  },
  {
    ...
    prerequisites: ["Build 1"],
    instructions: [
      "Write `checkConstraint(weight, max)` returning `{ok, violationPP?}`.",
  ...
  },
  {
    ...
    prerequisites: ["A handful of short strings", "Embeddings optional — word-overlap works as a fallback"],
    instructions: [
      "Make an array of 4 short chunks.",
  ...
  },
  {
    ...
    prerequisites: ["A constraint shape `{type, sector, value}`"],
    instructions: [
      "Define the constraint shape and an enum of allowed `type` values.",
      "`schemaValid`: `type` in the enum and `value` is a number.",
      "`businessValid`: `0 <= value <= 1`. Test `{type:'sector_max', sector:'Tech', value:-5}`.",
    ],
  ...
  },
  {
    ...
    prerequisites: ["Build 1"],
    instructions: [
      "`state = {}`; `research` writes `state.findings`.",
      "`risk` reads `findings`, writes `state.risk`.",
  ...
  },
  {
    ...
    prerequisites: ["Build 5"],
    instructions: [
      "`research()` → 'bullish', `risk()` → 'bearish' on the same input.",
  ...
  },
  {
    ...
    prerequisites: ["Build 4"],
    instructions: [
      "Constraints: Tech ≥ 0.40 and Tech ≤ 0.25.",
      "`check()` detects min>max and returns INFEASIBLE + the gap (15pp).",
